		...
		public static IDialog<string> dialog = Chain.PostToChain()
			...
			.Switch(
				new RegexCase<IDialog<string>>(new Regex("^hi", RegexOptions.IgnoreCase),
					(context, text) => Chain.ContinueWith(new GreetingDialog(), AfterGreetingContinuation)),
				new DefaultCase<string, IDialog<string>>(
					(context, text) => Chain.ContinueWith(FormDialog.FromForm(
						RoomReservation.BuildForm, FormOptions.PromptInStart), AfterGreetingContinuation))
			)
			.Unwrap()
			.PostToUser();
